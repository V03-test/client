var HttpUrlRequest = cc.Class.extend({
    ...
    ctor:function(url, reqId, param, onSuccess, onError){
        ...
        // cc.log("HttpUrlRequest==",this._url + "?" + this._body)
    },
...
var Network = {
    ...
    sypost : function(url,reqId,param,onSuccess,onError) {
        ...
        //}
        var req = new HttpUrlRequest(url,reqId,param,onSuccess,onError);
        var xhr = req.request();
        return xhr;
    },
    ...
    ipReq:function() {
        var url = HttpReqModel.getHttpUrlById(603);
    ...
    uploadUpdateErrorLog:function(_content) {
        ...
        NetworkJT.loginReqNew(100, {logMsg:errorLog}, function (data) {
        }, function (data) {
            if(data.message){
    ...
    requestHttp:function(url,data) {
        var _url = "http://192.168.1.35/api/index/bind_agent?package=demo";
        var data = {
            "payBindId":"962288",
            "userId":"120220",
        ...
        xhr.onreadystatechange = function () {
            if (xhr.readyState == 4) {
                if(xhr.status == 200){
                    ...
                    cc.log("data.message===",JSON.stringify(_data))
