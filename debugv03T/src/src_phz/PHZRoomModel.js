...
var PHZRoomModel = {
    ...
    getWanFaDesc:function(){
        ...
        if (this.wanfa == GameTypeEunmZP.SYZP){
            ...
            var infoArr = [];
            infoArr.push(this.intParams[2] == 3?"擂主支付":this.intParams[2] == 2?"房主支付":"AA支付");
    ...
    LYZP_ChiBianDaBianAndFangzhao:function(c){
        var self = this;
        PHZAlertPop.show("吃边打边，本局将只能胡黑胡、红胡、点朱胡、无胡，且同时放招，放招后不能吃牌、碰牌，确定出牌吗？",function(){
    ...
    LYZP_ChiBianDaBian:function(c){
        var self = this;
        PHZAlertPop.show("吃边打边，本局将只能胡黑胡、红胡、点朱胡、无胡，确定出牌吗？",function(){
    ...
    isShowChongPao:function(){
        var self = this;
        PHZAlertPop.showOnlyOk("只有听牌后才能打出这张牌",function(){
            //把打出去的牌还原回来，直接拿上一次的手牌数据来还原
            if(self.lastMineSortedJson) {
                self.nextSeat = self.mySeat;
                WCPHZMineLayout.restoreCard(self.lastMineSortedJson);
