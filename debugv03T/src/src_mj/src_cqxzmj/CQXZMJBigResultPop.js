 ...
 */
var CQXZMJBigResultPop = BasePopup.extend({
    user:null,
    ...
    refreshSingle:function(widget,user){
        ...
        if (user.actionCount){
            ...
            mgCount = user.actionCount[4] || 0;
            cjCount = user.actionCount[5] || 0;
        }

        var itemArr = ["自摸次数","接炮次数","点炮次数","暗杠次数","明杠次数","查大叫"];
        ...
        var localPosArr = [480,420,360,300,240,180];//[475,405,335,255,185]
        for(var i = 0;i<6;++i){
            if(itemArr[i]){
        ...
        ccui.helper.seekWidgetByName(widget,"Label_zm_3").setString(mgCount+"");
        ccui.helper.seekWidgetByName(widget,"Label_zm_4").setString(cjCount+"");
        ...
        if (MJRoomModel.isCreditRoom()) {
            ...
            //ccui.helper.seekWidgetByName(widget,"Label_credit").setString(""+credit);//擂台分
            //ccui.helper.seekWidgetByName(widget,"Label_credit").setString(""+user.ext[10]);//擂台分
    ...
    selfRender: function () {
        ...
        this.getWidget("wanfa_2").setString("重庆血战麻将");
        ...
        this.getWidget("Label_wf").setString("");
        ...
        if (MJRoomModel.roomName){
            var string = "";
            string = MJRoomModel.roomName + "   亲友苑ID：" + ClosingInfoModel.ext[0];
