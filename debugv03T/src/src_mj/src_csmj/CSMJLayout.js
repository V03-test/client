...
var CSMJLayout = cc.Class.extend({
    ...
    outCardTingPai:function(info){
        ...
        for(var i=0;i<this.mahjongs1.length;i++){
            ...
            for(var j=0;j < pushOutArray.length; j++){
                if (pushOutArray[j].majiangId && curVo.c == pushOutArray[j].majiangId){
                    ...
                    if(MJRoomModel.yymjTing){
                        isBool = true;
                    }
                }
            }
            var isHasChu = this.data3 && this.data3.length > 0;
            if(MJRoomModel.wanfa == GameTypeEunmMJ.YYMJ && MJRoomModel.yymjTing){// && MJRoomModel.yymjHasSet
                if(!isHasChu){
        ...
        //MJRoomModel.yymjHasSet = true;
    },

    setYYMJTingMoPai:function(){
        if(MJRoomModel.wanfa == GameTypeEunmMJ.YYMJ && MJRoomModel.yymjTing){// && MJRoomModel.yymjIsThrowCard
            //var isZhuang = MJRoomModel.banker == MJRoomModel.mySeat;
            var isHasChu = this.data3 && this.data3.length > 0;
            if(isHasChu){/** 不是首次摸牌*/
                for(var i=0;i<this.mahjongs1.length;i++) {
                    var mahjongs = this.mahjongs1[i];
    ...
    setColorNormal:function(){
        if(MJRoomModel.wanfa == GameTypeEunmMJ.YYMJ && MJRoomModel.yymjTing){
            for(var i=0;i<this.mahjongs1.length;i++) {
                var mahjongs = this.mahjongs1[i];
    ...
    getCardNumById:function(vo){
        if(MJRoomModel.wanfa == GameTypeEunmMJ.YYMJ){/** 如果是杠过的牌 **/
            for(var j = 0;j < this.data2.length;++j){
                var tempData = this.data2[j];
                if(tempData.action == MJAction.GANG || tempData.action == MJAction.AN_GANG){
            ...
            for(var i=0;i<this.data2.length;i++){
                ...
                if(innerObj.action==MJAction.PENG){
                    ...
                    if(curVo0.t==vo0.t&&curVo0.n==vo0.n){
                        ...
                        if(MJRoomModel.wanfa == GameTypeEunmMJ.GDCSMJ && action==MJAction.GANG){//广东潮汕麻将，明杠箭头指向自己
    ...
    refreshP1:function(data){
        ...
        for(var j=0;j<data.length;j++){
            if (((MJRoomModel.wanfa != GameTypeEunmMJ.GDCSMJ && data[j].i == 201) ||data[j].wang == 1) && data[j].m != 1 ){
                newData.push(data[j]);
            }
        }
        for(var j=0;j<data.length;j++){
        ...
        if(MJRoomModel.isMoneyRoom() && this.showAction){
            ...
            for(var i = 0;i<newData.length;i++){
                ...
                mjVo.a = 1;
                var card = new CSMahjong(MJAI.getDisplayVo(this.direct,2),mjVo);
    ...
    refreshP2:function(data){
        ...
        for(var i=0;i<data.length;i++) {
            ...
            for(var j=0;j<innerArray.length;j++){
                ...
                if(MJRoomModel.wanfa == GameTypeEunmMJ.GDCSMJ && innerAction == MJAction.GANG && innerObject.huxi > 0 && j==1){
                    card.showPengDir(innerObject.huxi);
                }
    ...
    refreshP4: function(data) {
        ...
        for(var i=0;i<data.length;i++) {
            ...
            if (this.direct != 1 && innerAction == 1) {//自摸
                if(MJRoomModel.wanfa != GameTypeEunmMJ.YYMJ){/** 益阳麻将不区分自摸放炮显示 */
                    vo.a = 1;
                }
            } else { //放炮
                //需要显示箭头
    ...
    onShowDesktopSameCards:function(card){
        ...
        if(MJRoomModel.wanfa == GameTypeEunmMJ.GDCSMJ){//手牌中相同的也要标记
            for (var i = 0; i < this.mahjongs1.length; i++) {
                var cardVo = this.mahjongs1[i]._cardVo;
    ...
    onRemoveLastSameCards:function(){
        ...
        if(MJRoomModel.wanfa == GameTypeEunmMJ.GDCSMJ){//手牌中相同的也要去掉
            for (var i = 0; i < this.mahjongs1.length; i++) {
                this.mahjongs1[i].removeGreyBg();
            }
        }
