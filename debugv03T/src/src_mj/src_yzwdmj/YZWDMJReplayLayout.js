...
var YZWDMJReplayLayout = cc.Class.extend({
    ...
    refreshP1:function(data){
        ...
        if (MJReplayModel.wanfa == GameTypeEunmMJ.AHMJ){
            if (MJReplayModel.ahmj_wangID && MJReplayModel.ahmj_wangID != -1){
                ...
                if (MJReplayModel.intParams[4] == 1){//四王
                    ...
                    // cc.log("first_wang =",first_wang,"second_wang =",second_wang);
                }
            }
        }else{
        ...
        for(var i=0;i<data.length;i++){
            if (MJReplayModel.wanfa == GameTypeEunmMJ.AHMJ){
                if (MJReplayModel.ahmj_wangID && MJReplayModel.ahmj_wangID != -1){
                    if (data[i].t ==  MJAI.getMJDef(MJReplayModel.ahmj_wangID).t && (data[i].n == first_wang || data[i].n == second_wang)){
                        data[i].wang = 1;
                    }
                }
            }else if(MJReplayModel.wanfa == GameTypeEunmMJ.TJMJ){
                if (ClosingInfoModel.ext && ClosingInfoModel.ext[25] && ClosingInfoModel.ext[26]){
                    if (data[i].t ==  MJAI.getMJDef(ClosingInfoModel.ext[26]).t && (data[i].n == first_wang)){
                        data[i].wang = 1;
                    }
                }
            }
        ...
        for(;i<newData.length;i++){
            ...
            if(MJReplayModel.wanfa == GameTypeEunmMJ.AHMJ){
                ...
                card = new YZWDMahjong(MJAI.getDisplayVo(this.direct,place),newData[i]);
            }
