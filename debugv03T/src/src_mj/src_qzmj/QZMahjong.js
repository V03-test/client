 ...
 */
var QZMahjong = ccui.Widget.extend({
    ...
    onTouchHandler:function(obj,type){
        ...
        if(MJRoomModel.localThrowCard && this._cardVo.noThrow){
            return false;
        }

        if(MJRoomModel.localThrowCard){
            var localVo = MJAI.getMJDef(MJRoomModel.localThrowCard);
            if(localVo && localVo.t == this._cardVo.t && localVo.n == this._cardVo.n){
        ...
        if(MJRoomModel.qzmjTing && !this._isCanThrow){//全州麻将听牌锁牌(只锁不可以听的牌)
            return false;
        }
        if(MJRoomModel.nextSeat!=MJRoomModel.mySeat || (MJRoomModel.selfAct.length>0 && MJRoomModel.selfAct[5] != 1))
            return false;
        ...
        if(type == ccui.Widget.TOUCH_BEGAN){
            if(!this.selected){
                ...
                if(MJRoomModel.isTingSelecting){
                    ...
                    if(this._cardVo.chunwang == 1 && MJRoomModel.wanfa == GameTypeEunmMJ.KWMJ){
                        self.selected = false;
                        self.x = self.initX;
                        self.y = self.initY;
                    }else{
    ...
    setCanThrow:function(isBool){
        ...
        if(isBool){
            ...
            //if(this._zi){
            //    this._zi.setColor(cc.color(255,255,255));
            //}
        }else{
            this._bg.setColor(cc.color(200,200,200));
            //if(this._zi){
            //    this._zi.setColor(cc.color(200,200,200));
            //}
