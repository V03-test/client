...
var ClubAgentManagePop = BasePopup.extend({
    ...
    selfRender: function() {
        ...
        this.Label_7.setString("你正在对“"+targetUserId+"”玩家进行奖赏分成操作");
    ...
    loadCreditConfig:function(){
        ...
        NetworkJT.loginReqNew("groupCreditAction", "loadCreditConfig", {
    ...
    updateCreditConfig:function(configs){
        ...
        if (configs && configs.length > 0){
            NetworkJT.loginReqNew("groupCreditAction", "updateCreditConfig", {
                groupId:ClickClubModel.getCurClubId(),
                userId:PlayerModel.userId,
                sessCode:PlayerModel.sessCode,
                targetUserId:targetUserId,
    ...
    showRuleText:function(){
        var str = "奖赏分成规则："	+
            ...
            "\n2、若为大赢家分成模式，则满足可设置给下级的分成值≤可分配的奖赏分（擂主可设置下级的分成值≤初始奖赏分；"+
            "\n3、若为参与分成模式，擂主给合伙人分配的分成值需满足 给合伙人分成值*玩法人数≤初始奖赏分，合伙人对下级门主以及下级门主对应给下级的分成值≤可分配奖赏分；"+
            "\n4、可针对各个玩法单独进行给下级奖赏分分成值设定，也可以批量设定；"+
            "\n5、批量设定分成值最大只可设定为所有玩法中可分配的最低奖赏分；" +
            "\n6、若上级修改了分配值后，分成界面将被修改过得玩法数据列的奖赏分数值变为红色，提示玩家即使更新设定；" +
