...
var PyqChenYuan = BasePopup.extend({
    ...
    selfRender:function(){
        ...
        this.inputId = new cc.EditBox(inputIdImg.getContentSize(),new cc.Scale9Sprite("res/ui/bjdmj/popup/light_touming.png"));
        ...
        this.inputBox_hhr.setPlaceHolder("查询馆主ID");
        this.inputBox_hhr.setPlaceholderFont("",36);
        this.inputBox_hhr.setPlaceholderFontColor(cc.color("#2155b5"));
    ...
    getClubInformateListData:function(curPage){
        curPage = curPage || 1;
        var self = this;
        //这协议定的我醉了 身份是0或者1(副擂主或者擂主)传1给后台 身份是2 传0给后台。我就不能传身份给你么？！
        NetworkJT.loginReqNew(416, {
    ...
    getClubUserListAdmin:function(userId,userId_hhr){
        ...
        //我在当前俱乐部的身份 1擂主 2副擂主 5000董事 10000主管 20000副擂主 30000合伙人
        var allRoleArr = [
            {name:"所有成员",userRole:0,userId:0,userName:""},
            {name:"擂主",    userRole:1,userId:0,userName:""},
            {name:"副擂主",  userRole:2,userId:0,userName:""},
            {name:"董事",    userRole:5000,userId:0,userName:""},
            {name:"主管",    userRole:10000,userId:0,userName:""},
            {name:"副擂主",    userRole:20000,userId:0,userName:""},
            {name:"合伙人",    userRole:30000,userId:0,userName:""}
        ];
        ...
        NetworkJT.loginReqNew(439,params , function (data) {
    ...
    onSortsClick:function(){
        ...
        var img1 = "res/res_ui/qyq/common/commonButton/jts2.png";
        var img2 = "res/res_ui/qyq/common/commonButton/jtx2.png";
    ...
    onClickUserRoleBtn:function(sender){
        ...
        this.orderByField = 1;
        this.orderByType = 0;
        this.btn_timeSort.loadTexture(" res/res_ui/qyq/common/commonButton/jtx1.png")
    ...
    getClubUserListNextLevel:function(curPage , searchNameOrId,targetUserId){
        ...
        //sy.scene.showLoading("获取成员信息");
        NetworkJT.loginReqNew(441,params , function (data) {
            //sy.scene.hideLoading();
            if (data && data.code == 0 && data.message) {
    ...
    getClubSearchUserList:function(curPage , searchNameOrId){
        ...
        //sy.scene.showLoading("获取成员信息");
        NetworkJT.loginReqNew(427,params , function (data) {
            sy.scene.hideLoading();
            if (data && data.code == 0 && data.message) {
                if(data.message.userList.length > 0){
                    self.curPage = curPage;
    ...
    onClickInvitePlayer:function(){
        ...
        obj.title = "请加入亲友苑ID:"+"["+ClickClubModel.getCurClubId()+"]";
    ...
    onClickTopBtn:function(sender){
        ...
        if(sender == this.btn_cygl){
            this.getWidget("layerBg").loadTexture("res/res_ui/qyq/common/commonBg/bgs.jpg");
        }else{
            this.getWidget("layerBg").loadTexture("res/res_ui/qyq/common/commonBg/bg.jpg");
        }
    ...
    setCyglScroll:function(data,curTargetUserId){
        ...
        var allRoleArr = [
            ...
            {name:"副擂主",    userRole:20000},
            {name:"合伙人",    userRole:30000},
        ...
        for(var i = 0;i<listNum;++i){
            ...
            //var userRole = data[i].userRole;//玩家等级 0 擂主 1 副擂主 2 普通人员
            //if(userRole == 0){
            ...
            if(data[i].isOnLine){
                img_online.loadTexture("res/res_ui/qyq/hall/img_zaixian.png");
            }else{
                img_online.loadTexture("res/res_ui/qyq/hall/img_lixian.png");
            }
    ...
    showLayerTip:function(parent,str){
        ...
        tip.setColor(cc.color(101,11,12));
...
var MemberItemPop = BasePopup.extend({
    ...
    onClickChangeRole:function(){
        if(this.itemData.userRole == 1){
            this.optUser("downRole","确定撤销该成员的副擂主职位吗？");
        }else{
            this.optUser("upRole","确定授予该成员副擂主职位吗？");
        }
    },
    ...
    optUser:function(type,tipStr){
        ...
        var netType = 148;
        if(type == "upRole"){//设为副擂主
            params.userRole = 1;
        }else if(type == "downRole"){//取消副擂主
            params.userRole = 2;
            ...
            params.userState = 1;
        }else if(type == "giveLeader"){//转让擂主
            params.userRole = 0;
        }

        AlertPop.show(tipStr,function() {
            NetworkJT.loginReqNew(netType, params, function (data) {
                if (data) {
                    FloatLabelUtil.comText(data.message);
                    //调用父节点的刷新
