...
var UserLoginLayer = BasePopup.extend({
	...
	showType:function(type){
		if(type==0){
			...
			this.getWidget("Image_title").loadTexture("res/res_ui/login/title_zhdl.png")
		}else{
			this.registerMc.visible=true;
			this.loginMc.visible=false;
			this.getWidget("Image_title").loadTexture("res/res_ui/login/title_zczh.png")
		}
	}
});
...
//		}else{
		...
		var k = md5(sign);
		var self=this;
		Network.loginReqNew(605,{p:"self",c:c,t:time,k:k},
				function(obj){
					sy.scene.hideLoading();
					if(!obj.user){
						AlertPop.showOnlyOk("注册失败,返回user参数为空");
