var AgentManagePop = BasePopup.extend({
    ...
    initSetUpData:function(){
        ...
        this.inputBox1.setPlaceHolder("请输入亲友苑ID");
    ...
    onCheckUserId:function(){
        ...
        var self = this;
        var url = HttpReqModel.getHttpUrlById(502);
        // var url = csvhelper.strFormat(SyConfig.REQ_URL,"qipai","getBindOneMsg");
        if(userId == ""){
            FloatLabelUtil.comText("玩家ID不能为空");
            return;
        }
        Network.sypost(url,502,{payBindId:PlayerModel.userId,armId:userId},
            function(data){
                var tempData = [];
    ...
    onSetUpClick:function(){
        ...
        if(beforeId == ""){
            FloatLabelUtil.comText("亲友苑ID不能为空");
            return;
        }
        if(afterId == ""){
    ...
    getUserData:function(){
        ...
        var url = HttpReqModel.getHttpUrlById(501);
        Network.sypost(url,501,{payBindId:userId},
            function(data){
    ...
    onItemClick:function(obj){
        ...
        AlertPop.show("是否给ID为"+data.userId +"的玩家开启亲友圈？",function(){
            var url = HttpReqModel.getHttpUrlById(506);
            // var url = csvhelper.strFormat(SyConfig.REQ_URL,"qipai","openCreateGroup");
            Network.sypost(url,506,{payBindId:PlayerModel.userId,armId:data.userId},
                function(data){
