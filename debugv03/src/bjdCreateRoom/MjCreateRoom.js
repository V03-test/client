...
var MjCreateRoom = BasePopup.extend({
    ...
    limitOpenGame:null,
    ctor:function(clubData,tempType){
    ...
    selfRender:function(){
        ...
        if(this.clubData){
            ...
            var bgSpr = new cc.Scale9Sprite("res/res_ui/qyq/common/commonKuang/inputBg.png",null,cc.rect(30,30,400,30));
            bgSpr.setContentSize(tempSize);
            this.inputName = new cc.EditBox(tempSize,bgSpr);
            this.inputName.x = cc.winSize.width/2 - 440;
    ...
    clubBagCreate:function(){
        ...
        if (ClickClubModel.getClubIsOpenCredit() && !this.bisaiSelect.isSelected()){
            FloatLabelUtil.comText("已开启擂台配置,必须勾选擂台房选项");
            return;
        }
        ...
        if (this.clubData.modeId) {
            ...
            self.onSaveChoose();
            //1创建俱包间，0非创建包间
            //NetworkJT.loginReqNew("groupAction", "createGroup", {groupName: bagName,groupLevel:1,
    ...
    changeBagName:function(name){
        ...
        NetworkJT.loginReqNew(309, {groupName: name,groupMode:0,
    ...
    onSaveChoose:function(){
        ...
        if (this.clubData.modeId) {
            ...
            NetworkJT.loginReqNew(310, params, function (data) {
                if (data) {
            ...
            NetworkJT.loginReqNew(300, params, function (data) {
                if (data) {
    ...
    showBisaiSelect:function(){
        ...
        contentNode.setPosition(cc.winSize.width/2 + 560,150);
        ...
        this.bisaiSelect = new SelectBox(2,boxStr,true);
        ...
        var tipBg = new cc.Scale9Sprite("res/res_ui/qyq/common/commonKuang/xiaoxiDi.png");
        tipBg.setContentSize(cc.size(1000,560));
        tipBg.setPosition(-520,400);
        tipBg.setVisible(false);
        contentNode.addChild(tipBg,1);

        var str = "1、擂台房勾选后开始设定擂台房限制；\n" +
            "2、参与最低擂台分：玩家擂台分低于设定值时不可进入房间；\n" +
            "3、踢出最低擂台分：牌局中当任意玩家擂台分低于设定值时将提前解散房间；\n" +
            "4、奖赏擂主擂台分：固定奖赏，牌局结算后大赢家或所有赢家自动奖赏擂主设定的擂台分；" +
            "比例奖赏，牌局结算后打赢大赢家或所有赢家自动奖赏擂主赢分的设定比例的擂台分；\n" +
            "5、底分：牌局中获得的输赢积分*底分=输赢擂台分；\n" +
            "6、参与最低擂台分必须大于踢出最低擂台分；" ;

        var tipLabel = UICtor.cLabel(str,36,cc.size(tipBg.width-40,tipBg.height-20),cc.color("#90371f"),0,1);
        tipLabel.setPosition(tipBg.width/2,tipBg.height/2);
        tipBg.addChild(tipLabel);

        if(!ClickClubModel.getClubIsGold()){
            var tipBtn = UICtor.cBtn("res/res_ui/qyq/common/commonButton/tip.png");
        ...
        var changeBtn = UICtor.cBtn("res/res_ui/qyq/wanfa/xg.png");
        changeBtn.loadTexturePressed("res/res_ui/qyq/wanfa/xg.png");
        ...
        changeBtn.setScale(0.6);
        changeBtn.setPosition(85,-65);
    ...
    setItemSelect:function(item){
        ...
        item.loadTextureNormal("res/res_ui/qyq/common/commonButton/anniu1.png",ccui.Widget.LOCAL_TEXTURE);
        item.getChildByName("Label_game_type").setColor(cc.color("#b23b00"))

        if(this.curItem){
            this.curItem.loadTextureNormal("res/res_ui/qyq/common/commonButton/anniu2.png",ccui.Widget.LOCAL_TEXTURE);
            this.curItem.getChildByName("Label_game_type").setColor(cc.color("#876023"))
        }
