...
var AHMJSmallResultPop = BasePopup.extend({
    ...
    selfRender: function () {
        ...
        if(MJRoomModel.nowBurCount == MJRoomModel.totalBurCount
            || (MJRoomModel.wanfa == GameTypeEunmMJ.CXMJ?(this.data.ext[10] == 1):(this.data.ext[9] == 1))
            || this.data.isBreak){
    ...
    AHMJShowBirdsInHand: function(){
        ...
        for(var i = 0;i<birdData.length;++i) {
            ...
            for (var j = 0; j < dirBird.length; ++j) {
                ...
                var size = card.getContentSize();
                card.scale = _scale;
                card.x = 1500 - (j + 1) * (size.width * _scale - 0.5);
                card.y = 75;
                if(MJRoomModel.wanfa == GameTypeEunmMJ.KWMJ){
                    _scale = 0.4;
                    card.scale = _scale;
                    card.x = 1500 - (j%3 + 1) * (size.width * _scale - 0.5);
    ...
    showBirds: function() {
        ...
        if (birdList){
            for(var j=0;j<birdList.length;j++) {
                ...
                var _scale = 0.5;
                card.scale = _scale;
                card.x = (size.width * _scale - 1.5) * j - 10;
                card.y = 10;
