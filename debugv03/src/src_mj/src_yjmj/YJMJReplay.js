...
var YJMJReplay = BaseLayer.extend({
    ...
    showLiangWangAni:function(id,kingCardId){
        ...
        this.wang_card = new YJMahjong(MJAI.getDisplayVo(1, 1), vo);
        ...
        if(MJReplayModel.wanfa == GameTypeEunmMJ.TJMJ){
            ...
            otherVo.wang = 1;
            var wangpai = new YJMahjong(MJAI.getDisplayVo(1, 1), otherVo);
            wangpai.x = 135;
    ...
    showTJMJGang:function(acts){
        if(acts.length > 0){
            ...
            for(var i = 0;i<acts.length;++i){
                ...
                var card = new YJMahjong(MJAI.getDisplayVo(1, 1), MJAI.getMJDef(id));
                card.x = i * 105 + 10;
                card.y = 5;
