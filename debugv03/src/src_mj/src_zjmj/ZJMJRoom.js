...
var ZJMJRoom = BaseRoom.extend({
    ...
    selfRender:function(){
        ...
        this.addCustomEvent(SyEvent.CS_GANG_MAJIANG,this,this.onCSGang);//添加杠摸牌消息
        ...
        // var gameNameImg = new cc.Sprite("res/res_mj/res_hzmj/zjmjRoom/zjmj.png");
        // var x = 960;
        // var y = 740;
        ...
        MJRoomModel.isYAOGANG = false;/** 是否摇杠 */
    ...
    onCSGang:function(event){
        ...
        //if(message.seat == MJRoomModel.mySeat && !MJRoomModel.isYAOGANG){
        //    MJRoomModel.isYAOGANG = true;/** 摇杠 */
        //}
        ...
        if(isHas){
            if(delay == 0){
                ...
                this.refreshButton(acts[0].selfAct);
            }else{
                setTimeout(function(){
                    //PopupManager.addPopup(new MJGangPop());
                    MJRoomModel.lastMoMaJiang = parseInt(acts[0].majiangId);
                    MJRoomModel.needAutoLetOutId = MJRoomModel.lastMoMaJiang;
                    self.refreshButton(acts[0].selfAct);
                },delay);
            }
        }
    },
    ...
    csGangMoPai:function(seat){
        ...
        if(seat == MJRoomModel.mySeat){
            ...
            for(var i = 0;i<acts.length;++i){
                ...
                if(!(selfAct && selfAct.length > 0 && (selfAct[2] || selfAct[3] || selfAct[5] || selfAct[6]))){
    ...
    onCheckResult:function(){
        if(this.resultData){
            var mc = new ZJMJSmallResultPop(this.resultData);
            PopupManager.addPopup(mc);
        }
    },
    ...
    onSelectMajiang:function(event){
        ...
        var ids = data.ids;
        if(MJAction.YAO_GANG == action){
            var myLayout = this.getLayout(MJRoomModel.getPlayerSeq(PlayerModel.userId));
            var isHasAllIds = myLayout.isContainIds(ids);
            if(isHasAllIds && ids.indexOf(0) === -1 && ids.length === 4){//摇暗杠
                MJRoomModel.sendPlayCardMsg(15,ids);
            }else{
    ...
    onOver:function(event){
        ...
        this.overTimeout = setTimeout(function(){//延迟弹出结算框
            ...
            var mc = new ZJMJSmallResultPop(data);
            PopupManager.addPopup(mc);
    ...
    initData:function(){
        ...
        for(var i=0;i<players.length;i++){
            ...
            if(p.userId ==PlayerModel.userId){//自己的状态处理
                ...
                MJRoomModel.isYAOGANG = p.ext[6] == 1;/** 摇杠 */
                ...
                if(p.handCardIds.length%3==2){
                    ...
                    if(MJRoomModel.isYAOGANG){//直接出牌
                        MJRoomModel.needAutoLetOutId = p.handCardIds[p.handCardIds.length-1];
                        MJRoomModel.lastMoMaJiang = MJRoomModel.needAutoLetOutId;
    ...
    onOperate:function(obj,isAlert){
        ...
        switch (temp){
            ...
            case MJAction.YAO_GANG://芷江麻将摇杠
                //cc.log("MJAction.YAO_GANG==========",MJAction.YAO_GANG)
                var ids = [];
                var myLayout = this.getLayout(MJRoomModel.getPlayerSeq(PlayerModel.userId));
    ...
    refreshButton:function(selfAct){
        ...
        if(selfAct.length>0){
            ...
            var textureMap = {
                ...
                6:{t:"res/res_mj/res_hzmj/zjmjRoom/mj_btn_yaogang.png",v:14},
            ...
            for(var i=0;i<selfAct.length;i++){
                ...
                if(temp==1 && tm){
                    ...
                    if(i == 6){//摇杠按钮比较大，距离拉开一点
                        this.hbtns[rIndex].x = localStartX - offsetX * localCount - 40;
                    }
                    ++localCount;
            ...
            if (selfAct[5] == 1 && MJRoomModel.isYAOGANG && MJRoomModel.lastMoMaJiang >= 201 && MJRoomModel.lastMoMaJiang < 205){
                var mineHandCards = this.getLayout(1).getPlace1Data();
                //摇杠有炮必胡
                if (mineHandCards.length%3 == 2){
    ...
    onGetMajiang:function(event){
        ...
        if(MJRoomModel.isGSMJ() || MJRoomModel.isGuCang()){
            ...
            if(seat==MJRoomModel.mySeat && id>0){//直接出牌
                if(MJRoomModel.isYAOGANG){
                    MJRoomModel.needAutoLetOutId = id;
                    MJRoomModel.lastMoMaJiang = id;
                    if(selfAct.length==0){
                        setTimeout(function () {
                            MJRoomModel.chuMahjong(id);
                        }, 500)
                    }
                }
            }
    ...
    onLetOutCard:function(event){
        ...
        //if(seat==MJRoomModel.mySeat && !MJRoomModel.isYAOGANG && action == MJAction.YAO_GANG){
        //    MJRoomModel.isYAOGANG = true;/** 摇杠 */
        //}
        MJRoomModel.isTingSelecting=false;
