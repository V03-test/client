 ...
 */
var CQXZMJRoom = BaseRoom.extend({
    layouts:{},
    initCoords:{},
    COUNT_DOWN:15,
    ...
    selfRender:function(){
        ...
        this.addCustomEvent(SyEvent.CQXZMJ_START_HUANZHANG , this,this.start_huanzhang);
        this.addCustomEvent(SyEvent.CQXZMJ_SUCESS_HUANZHANG , this,this.sucess_huanzhang);
        this.addCustomEvent(SyEvent.CQXZMJ_CHECK_HUANZHANG , this,this.check_huanzhang);
        this.addCustomEvent(SyEvent.CQXZMJ_FINISH_HUANZHANG , this,this.finish_huanzhang);
        this.addCustomEvent(SyEvent.CQXZMJ_START_DINGQUE , this,this.start_dingque);
        this.addCustomEvent(SyEvent.CQXZMJ_SUCESS_DINGQUE , this,this.sucess_dingque);
        this.addCustomEvent(SyEvent.CQXZMJ_FINISH_DINGQUE , this,this.finish_dingque);
        this.addCustomEvent(SyEvent.CQXZMJ_SCOREBOARD , this,this.refresh_scoreboard);
        ...
        this.Panel_huanzhang = this.getWidget("Panel_xuanzhang");
        this.huanzhang_direct = new cc.LabelTTF("本局为顺时针换张","Arial",44,cc.size(400, 65));
        this.huanzhang_direct.x = this.Panel_huanzhang.width/2 + 50;
        this.huanzhang_direct.y = - 20;
        this.Panel_huanzhang.addChild(this.huanzhang_direct);
        if(MJRoomModel.renshu == 2){
            this.huanzhang_direct.visible = false;
        }

        this.Button_huanzhang = this.getWidget("Button_xuanzhang"); 
        var Image_102 = ccui.helper.seekWidgetByName(this.Panel_huanzhang,"Image_102");
        var check_num = MJRoomModel.intParams[5]?4:3;
        Image_102.loadTexture("res/res_mj/res_cqxzmj/cqxzmjRoom/xuanzhang_"+check_num+".png");
        UITools.addClickEvent(this.Button_huanzhang,this,this.send_huanzhang);

        this.Panel_dingque = this.getWidget("Panel_dingque");
        this.Button_tiao = this.getWidget("Button_tiao"); 
        this.Button_tiao.temp = 1;
        this.Button_tong = this.getWidget("Button_tong"); 
        this.Button_tong.temp = 2;
        this.Button_wan = this.getWidget("Button_wan"); 
        this.Button_wan.temp = 3;
        UITools.addClickEvent(this.Button_tiao,this,this.send_dingque);
        UITools.addClickEvent(this.Button_tong,this,this.send_dingque);
        UITools.addClickEvent(this.Button_wan,this,this.send_dingque);
        ...
        var Button_zhanji = this.getWidget("Button_zhanji");
        UITools.addClickEvent(Button_zhanji,this,this.showZhanji);
        this.zhanji_score = this.getWidget("score_1");
        this.Panel_zhanji = this.getWidget("Panel_zhanji");
        ...
        cc.eventManager.addListener(cc.EventListener.create({
            ...
            onTouchBegan: function(touch,event){
                this.getWidget("Image_zhanji").setVisible(false);
            }.bind(this)
        }),node);

    },
    refresh_scoreboard:function(message){
        var data = message.getUserData();
        // cc.log("data =",JSON.stringify(data));
        this.zhanji_score.setString(data.countPoint);
        this.Panel_zhanji.removeAllChildren();
        for (var i = 0; i < data.aor.length; i++) {
            var string = "";
            var actLabel = new cc.LabelTTF("","Arial",44,cc.size(320, 65));
            var height_index = Math.floor(i/2);
            actLabel.setPosition(cc.p(-260 +440*(i%2+1),280+(height_index*-60)));
            actLabel.setColor(cc.color("#FFAC6956"));
            var scoreLabel = new cc.LabelTTF(data.aor[i].point,"Arial",44,cc.size(320, 65));
            scoreLabel.setPosition(cc.p(10 + 440*(i%2+1),280+(height_index*-60)));
            scoreLabel.setColor(cc.color("#FFAC6956"));

            var direct = MJRoomModel.getPlayerSeq(data.aor[i].operatorId,MJRoomModel.mySeat,data.aor[i].operatorSeat);
            if(data.aor[i].act == 1 ){
                string = "刮风";
            }else if(data.aor[i].act == 2){
                string = "下雨";
            }else if(data.aor[i].act == 3){
                string = "自摸";
            }else if(data.aor[i].act == 4){
                string = "点炮";
            }else if(data.aor[i].act == 5){
                string = "接炮";
            }else if(data.aor[i].act == 6){
                string = "转移";
            }else if(data.aor[i].act == 7){
                string = "查叫";
            }else if(data.aor[i].act == 8){
                string = "直杠";
            }else if(data.aor[i].act == 9){
                string = "退税";
            }  
            if(direct == 2){
                if(MJRoomModel.renshu == 2){
            ...
            actLabel.setString(string);
            this.Panel_zhanji.addChild(actLabel);
            this.Panel_zhanji.addChild(scoreLabel);
        }
    },
    showZhanji:function(){
        var Image_zhanji = this.getWidget("Image_zhanji");
        Image_zhanji.visible = !Image_zhanji.isVisible();
    },
    send_dingque:function(obj){
        // cc.log("obj =",JSON.stringify(obj.temp));
        sySocket.sendComReqMsg(2086,[obj.temp]);
    },
    start_dingque:function(message){
        MJRoomModel.cqxzmj_gameType = 2;
        var data = message.getUserData();
        // cc.log("start_dingque =",JSON.stringify(data));
        var self = this;
        self.Panel_dingque.visible = true;
        
        if(MJRoomModel.renshu == 2){
            this.Button_wan.visible = false;
        }
        this.showDingqueBtnAni(data.params);
    },
    sucess_dingque:function(message){
        var data = message.getUserData();
        // cc.log("sucess_dingque =",JSON.stringify(data))
        var userId = data.params[0];
        var p = MJRoomModel.getPlayerVo(userId)
        var direct = MJRoomModel.getPlayerSeq(userId,MJRoomModel.mySeat,p.seat);
        if(direct == 1){
            this.Panel_dingque.visible = false;
            this.Button_tiao.removeAllChildren();
            this.Button_tiao.stopAllActions();
            this.Button_tong.removeAllChildren();
            this.Button_tong.stopAllActions();
            this.Button_wan.removeAllChildren();
            this.Button_wan.stopAllActions();
            this.layouts[1].refreshP1();
        }
    },

    finish_dingque:function(message){
        var data = message.getUserData();
        // var arr = JSON.parse(data.strParams[0])
        this.Panel_dingque.visible = false;
        var userId = data.params[0];
        var type = data.params[1];
        var p = MJRoomModel.getPlayerVo(userId)
        var direct = MJRoomModel.getPlayerSeq(userId,MJRoomModel.mySeat,p.seat);
        this._players[p.seat].showDingQueImg(type);
        MJRoomModel.cqxzmj_gameType = 3;
        if(direct == 1){
            MJRoomModel.cqxzmj_dingqueType = type;
            this.layouts[1].refreshP1();
        }
    },
    
    showDingqueBtnAni:function(array){
        for (var i = 0; i < array.length; i++) {
            var tuijianImg = new cc.Sprite("res/res_mj/res_cqxzmj/cqxzmjRoom/dingque_tuijian.png");
            tuijianImg.x = 100;
            tuijianImg.y = 220;
            var action = cc.repeatForever(cc.sequence(cc.scaleTo(0.5,1.2),cc.scaleTo(0.5,1.1)));
            if(array[i] == 1){
                this.Button_tiao.addChild(tuijianImg);
                this.Button_tiao.runAction(action);
            }else if(array[i] == 2){
                this.Button_tong.addChild(tuijianImg);
                this.Button_tong.runAction(action);
            }else if(array[i] == 3){
                this.Button_wan.addChild(tuijianImg);
                this.Button_wan.runAction(action);
            }
        }
    },

    finish_huanzhang:function(message){
        MJRoomModel.cqxzmj_gameType = 2;
        var data = message.getUserData();
        cc.log("finish_huanzhang =",JSON.stringify(data));
        var cardData = this.layouts[1].transData(data.params,true);
        this.Panel_huanzhang.visible = false;
        var self = this;
        setTimeout(function(){
            self.layouts[1].refreshP1(cardData,true,data.strParams[0]);
        },500)
    },

    check_huanzhang:function(){
        var num = this.layouts[1].getUpCardsNum();
        var check_num = MJRoomModel.intParams[5]?4:3;
        if(num != check_num){
            this.Button_huanzhang.setBright(false);
        }else{
            this.Button_huanzhang.setBright(true);
        }
    },

    send_huanzhang:function(){
        var array = this.layouts[1].getHuanzhangCards();
        // cc.log("array =",JSON.stringify(array));
        sySocket.sendComReqMsg(2083,array);
    },


    start_huanzhang:function(message){
        var data = message.getUserData();
        this.Panel_huanzhang.visible = true;
        // cc.log("start_huanzhang =",JSON.stringify(data));
        //0 顺时针 1 逆时针 2 对家
        if(data.params[0] == 1){
            this.huanzhang_direct.setString("本局为逆时针换张")
        }else if(data.params[0] == 2){
            this.huanzhang_direct.setString("本局为对家换张")
        }
        var self = this;
        setTimeout(function(){
            MJRoomModel.cqxzmj_gameType = 1;
            self.layouts[1].huanzhang_cards_up(data.params);
        },100);
        
    },

    sucess_huanzhang:function(message){
        var data = message.getUserData();
        var userId = data.params[0];
        var p = MJRoomModel.getPlayerVo(userId)
        var direct = MJRoomModel.getPlayerSeq(userId,MJRoomModel.mySeat,p.seat);
        if(direct == 1){
            ...
            for (var i = 1; i < data.params.length; i++) {
                array.push(data.params[i]);
            }
            this.layouts[1].huanzhang_sucess(array);
        }
    },
    hideAction:function() {
    ...
    newUpdateFace:function(){//宽屏适配
        ...
        if(size.width > SyConfig.DESIGN_WIDTH){
            ...
            this.getWidget("Button_zhanji").x -= tempSize - offx;
    ...
    onShowHuPanel:function(isShowAllTime){
        ...
        if (huList && huList.length > 0){
            ...
            for (var i = 0; i < huList.length; i++) {
                ...
                var card = new CQXZMahjong(MJAI.getDisplayVo(1, 2), vo);
    ...
    onShowHuCardsByTingPai:function(event){
        ...
        if(huCards.length>0) {
            ...
            for (var i = 0; i < huCards.length; i++) {
                var vo = huCards[i];
                var card = new SYCQXZhjong(MJAI.getDisplayVo(1, 4), vo);
                card.x = orderNum * 27;
                card.y = 0;
    ...
    onCheckResult:function(){
        if(this.resultData){
            var mc = new CQXZMJSmallResultPop(this.resultData);
            PopupManager.addPopup(mc);
        }
    },
    ...
    onJixuFromResult:function(){
        this.Panel_8.visible = false;
        cc.log("ClosingInfoModel.ext[8] =",ClosingInfoModel.ext[8]);
        if(MJRoomModel.totalBurCount == MJRoomModel.nowBurCount || ClosingInfoModel.ext[8] == 1){
            var mc = new CQXZMJBigResultPop(this.resultData);
            PopupManager.addPopup(mc);
        }else {
    ...
    onCancelSelect:function(){
        if(MJRoomModel.cqxzmj_gameType == 3){
    ...
    onOver:function(event){
        ...
        for(var i=0;i<closingPlayers.length;i++){
            ...
            self._players[p.seat].hideHuimg();
            self._players[p.seat].hideDingqueImg();
            var seq = MJRoomModel.getPlayerSeq(p.userId,MJRoomModel.mySeat, p.seat);
            var handPais = ArrayUtil.clone(p.handPais);
            if(p.isHu){
                handPais.push(p.isHu);
            }
            // cc.log("p.handPais =",JSON.stringify(handPais));
            self.getLayout(seq).tanPai(handPais);
        }
        ...
        this.overTimeout = setTimeout(function(){//延迟弹出结算框
            ...
            var mc = new CQXZMJSmallResultPop(data);
            PopupManager.addPopup(mc);
            if(self.Panel_8)
                self.Panel_8.visible = true;
        },t);

        MJRoomModel.cqxzmj_gameType = 0;
    ...
    initData:function(){
        MJRoomModel.cqxzmj_gameType = 0;
        MJRoomModel.cqxzmj_dingqueType = 0;
        this.roomName_label.setString(MJRoomModel.roomName);
        this.initwanfaImg();      
        ...
        for(var i=0;i<players.length;i++){
            ...
            if(!isContinue){
                ...
                var banker = null;
                //if(p.seat==MJRoomModel.nextSseat)
                //    banker= p.seat;
                if(p.ext[9]!=0){
                    MJRoomModel.cqxzmj_gameType = 3;
                    cardPlayer.showDingQueImg(p.ext[9]);
                    if(seq == 1){
                        MJRoomModel.cqxzmj_dingqueType = p.ext[9];
                    }
                }else{
                    MJRoomModel.cqxzmj_gameType = 2;
                }
                ...
                if(p.ext[11] > 0 ){
                    cardPlayer.showHuimg(p.ext[11]);
                }else{
                    cardPlayer.hideHuimg();
                }
    ...
    showNiaoPanel:function(data){
        ...
        if (birdList && birdList.length > 0){
            for(var i = 0;i < birdList.length; i++) {
                ...
                var card = new SYMCQXZjong(MJAI.getDisplayVo(1, 1), vo);
                var size = card.getContentSize();
    ...
    onShowHuCards:function(huArray){
        ...
        if(huArray.length>0){
            ...
            for(var key in huArray){
                var vo = huArray[key];
                //vo.tingDisplay = 1;
                var card = new CQXZMahjong(MJAI.getDisplayVo(1,4),huArray[key]);
                card.x = orderNum*27;
                card.y = 0;
                this.Panel_ting.addChild(card);
                orderNum += 1;
    ...
    updateRemain:function(){
        ...
        var label = new RichLabel(cc.size(150,40));
    ...
    displaySelectMahjongs:function(action,resultArray){
        ...
        for(var i=0;i<resultArray.length;i++){
            ...
            for(var j=0;j<chiArr.length;j++){
                ...
                var mahjong = CQXZSYMahjong(MJAI.getDisplayVo(1,3),chiVo);
                mahjong.scale=scale;
    ...
    onLetOutCard:function(event){
        ...
        switch (action){
            ...
            case MJAction.HU://胡牌动作
                ...
                // cc.log("seat =",seat);
                if(direct != 1){
                    this.getLayout(direct).moPai(0);
                }else if(message.zimo != 1){
                    this.getLayout(direct).moPai(lastId);
                }
                this._players[seat].showHuimg(ext[0]);
                var prefix = "hu";//(message.zimo==1) ? "zimo" : "hu";
    ...
    smartFitlerMayHuPai:function(isReconect){
    	...
    	if(list.length>0){
            ...
            for(var key in list){
                var vo = list[key];
                //vo.tingDisplay = 1;
                var card = new CQXZMahjong(MJAI.getDisplayVo(1,4),list[key]);
                card.x = orderNum*27;
                card.y = 0;
                this.Panel_ting.addChild(card);
                orderNum += 1;
