/**
 * Created by zhoufan on 2016/7/22.//1
 */
var HZMahjong = ccui.Widget.extend({
    ...
    onTouchHandler:function(obj,type){
        ...
        if((MJRoomModel.isYAOGANG || (this._cardVo && this._cardVo.i == 201)) && MJRoomModel.wanfa == GameTypeEunmMJ.ZJMJ){/** 芷江麻将摇杠锁牌 **/
            return false;
        }
        ...
        if(type == ccui.Widget.TOUCH_BEGAN){
            if(!this.selected){
                ...
                if(MJRoomModel.isTingSelecting){
                    ...
                    }else if(MJRoomModel.wanfa == GameTypeEunmMJ.ZOUMJ){
                        if (this._cardVo && (this._cardVo.i == MJRoomModel.ext[5]
                            || this._cardVo.i == MJRoomModel.ext[13])) {
                            AlertPop.show("王牌，确定要出吗？", function () {
                                self.onChuMahjong();
                            }, function () {
                                self.selected = false;
    ...
    diplay:function(){
         ...
         *   HZMJ需要用
         */
        var isFirst = UITools.getLocalItem("pro003_mj_pm_reChange"+MJRoomModel.wanfa);
        if(!isFirst && MJRoomModel.wanfa == GameTypeEunmMJ.HZMJ){
            bgstyle = 3; //1 旧麻将 2 新麻将;
            UITools.setLocalItem("pro003_mj_pp"+MJRoomModel.wanfa,3);
            UITools.setLocalItem("pro003_mj_pm_reChange"+MJRoomModel.wanfa,1);
        }
        ...
        if (this._cardVo.wang == 1){
            ...
            if(MJRoomModel.wanfa == GameTypeEunmMJ.DZMJ || MJRoomModel.wanfa == GameTypeEunmMJ.ZOUMJ){
                this._bg.setColor(cc.color.YELLOW);
            }
        }
        //听牌的展示
