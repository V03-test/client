 ...
 */
var QZMJLayout = cc.Class.extend({
    gangPai:[],
        ...
        for(var i=0;i<this.mahjongs1.length;i++){
            ...
            for(var j=0;j < pushOutArray.length; j++){
                if (pushOutArray[j].majiangId && curVo.c == pushOutArray[j].majiangId){
                    ...
                    if(MJRoomModel.qzmjTing){
                        isBool = true;
                    }
                }
            }
            var isHasChu = this.data3 && this.data3.length > 0;
            if(MJRoomModel.qzmjTing){
                if(!isHasChu){
                    mahjongs.setCanThrow(isBool);
                }else{
                    if(this.mahjongs1.length % 3 === 2){
                        if(i < this.mahjongs1.length - 1){
                            mahjongs.setCanThrow(false);
                        }
                    }else{
                        mahjongs.setCanThrow(false);
                    }
    ...
    setQZMJTingMoPai:function(){
        ...
        var localVo = MJAI.getMJDef(MJRoomModel.localThrowCard);
        for(var i=0;i<this.mahjongs1.length;i++) {
            var mahjongs = this.mahjongs1[i];
            var voData = mahjongs.getData();
            if(localVo && localVo.t == voData.t && localVo.n == voData.n){
                mahjongs.setCanThrow(false);
                voData.noThrow = true;
            }else{
                mahjongs.setCanThrow(true);
                voData.noThrow = false;
            }
        }
    },

    clearNoThrow:function(){
        for(var i=0;i<this.mahjongs1.length;i++) {
            var mahjongs = this.mahjongs1[i];
            var voData = mahjongs.getData();
            if(voData.noThrow){
                mahjongs.setColorNormal();
                voData.noThrow = false;
            }
        }
    },
    ...
    getCardNumById:function(vo){
        for(var j = 0;j < this.data2.length;++j){
            var tempData = this.data2[j];
            if(tempData.action == MJAction.GANG){
                if(vo.i == tempData.cards[0].i){
                    return 4;
                }
            }else if(tempData.action == MJAction.AN_GANG){
                if(vo.i == tempData.cards[0].i && this.direct == 1){
                    return 4;
                }
            }
        }
        ...
        for(var i=0;i<this.mahjongs2.length;i++){
            ...
            var curVo = mahjongs.getData();
            if(this.direct != 1 && curVo.a == 1){//其他人的暗杠不计算
                continue;
            }
            if (curVo.i == vo.i){
                num = num +1;
    ...
    xiaohu:function(ids){
        ...
        for(var i=0;i<data.length;i++){
            var card = new QZMahjong(MJAI.getDisplayVo(this.direct,3),data[i]);
            if(this.direct==1){
                card.x = initVal+i*g;
    ...
    getnewDataForTCPFMJ:function(data){
        ...
        if(zhuanPai_index == -1){ //
            ...
            var _wangVo = MJAI.getMJDef(MJRoomModel.qzmj_wangID)
            ...
            var vo1 = MJAI.getMJDef(MJRoomModel.qzmj_wangID);
            vo1.n = first_wang;
            vo1.zhuan = 1;
            tempdata.push(vo1)
    ...
    refreshP1:function(data){
        ...
        for(var i=0;i<data.length;i++){
            if (cardVo && data[i].t == cardVo.t && data[i].n == nWang){
                data[i].wang = 1;
            }
        }
        var newData = [];
        ...
        var correctCoord = function(direct,card,i,zorder,isMopai){
            ...
            if((newData[i].hasOwnProperty("m") && card._cardVo.wang == 0) || i>12 || isMopai){//摸牌的处理
                if(direct==1){
                    card.x += 30;
                }
                if(direct==2)
        ...
        for(;i<newData.length;i++){
            newData[i].feiDisplay = 1;
            if (MJRoomModel.qzmj_wangID && MJRoomModel.qzmj_wangID != -1){
                if (newData[i].t ==  MJAI.getMJDef(MJRoomModel.qzmj_wangID).t && newData[i].n == nWang){
                    newData[i].wang = 1;
                }
            }
            var card = new QZMahjong(MJAI.getDisplayVo(this.direct,1),newData[i]);
        ...
        for(var i=0;i<teamData.length;i++) {
            ...
            for(var j=0;j<innerArray.length;j++){
                ...
                if(count<nowCards){
                    ...
                    card = new QZMahjong(MJAI.getDisplayVo(this.direct,2),innerVo);
                    this.mPanel.addChild(card);
                }
                if(j == 0){
                ...
                if(gangVo && j==1){
                    if(!card.getChildByTag(333)){
                        if(innerAction==MJAction.AN_GANG && this.direct!=1)//只有自己可以看到暗杠的牌
                            gangVo.a = 1;
                        var gang = new QZMahjong(MJAI.getDisplayVo(this.direct,2),gangVo);
                        if(this.direct==1)
                            gang.y = 20;
                        if(this.direct==2)
                            gang.y += 15;
    ...
    diplayP3ForTCGame:function(data){
        ...
        for(;i<teamData.length;i++){
            var card = new QZMahjong(MJAI.getDisplayVo(this.direct,3),teamData[i]);
            // card.removeHuImg();
    ...
    refreshP4: function(data,isBuhua) {
        ...
        for(var i=0;i<data.length;i++) {
            ...
            vo.huDisplay = 1;
            var card = new QZMahjong(MJAI.getDisplayVo(this.direct,4),vo);
            card.x = initVal+g*i;
            card.y = 0;
            if(this.direct==1){
                card.x = initVal-i*g;
