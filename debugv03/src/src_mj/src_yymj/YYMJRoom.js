...
var YYMJRoom = BaseRoom.extend({
    ...
    selfRender:function(){
        ...
        this.addCustomEvent("YYMJ_HAIDI",this,this.onCSHaidi);/** 益阳麻将海底牌 **/
        ...
        /**  益阳麻将锁牌 **/
        this.addCustomEvent("YYMJ_DINGPAI_CLOSE", this,this.onYYMJCloseCard);
        ...
        MJRoomModel.yymjIsCanChu = true;/** 益阳麻将锁牌(锁没有操作的玩家的牌)  **/
        MJRoomModel.yymjHasSet = false;//有没有设置听牌
        ...
        //this.yymjTipLabel = new cc.LabelTTF("","Arial",36,cc.size(320, 45));
        //this.yymjTipLabel.setAnchorPoint(0.5,0.5);
        //this.Panel_20.addChild(this.yymjTipLabel,1);
        //this.yymjTipLabel.y = this.jt.y - 120;
        //this.yymjTipLabel.x = this.jt.x;
    },
    ...
    onTingClick:function(event){
        ...
        if(params[1] == 1){
            ...
            if (p.seat == MJRoomModel.mySeat){
                MJRoomModel.yymjTing = true;//听牌玩家锁牌
            }else{
    ...
    onYYMJCloseCard:function(){
        MJRoomModel.yymjIsCanChu = false;
        //if(!this.tjmjButton_ting || (this.tjmjButton_ting && !this.tjmjButton_ting.visible)){
        //    this.yymjTipLabel.setString("等待其他玩家听牌...");
        //}
    },
    ...
    onXiaoHuPlayCard:function(){
        ...
        setTimeout(function(){//延时一下，应对开局时createTable消息后到的情况
            MJRoomModel.yymjIsCanChu = true;
            //self.yymjTipLabel.setString("");
    ...
    onCSHaidi:function(event){
        ...
        //}
        //cc.log(" message.params = ",JSON.stringify(message.params));
        if(message.params && message.params[0] > 0){
            var mjVo = MJAI.getMJDef(message.params[0]);
            var t = 500;
            var self = this;
            setTimeout(function(){
                var card = new CSMahjong(MJAI.getDisplayVo(1, 1), mjVo);
                card.x = SyConfig.DESIGN_WIDTH / 2;
                card.y = SyConfig.DESIGN_HEIGHT / 2;
                card.scale = 0.85;
                card.setAnchorPoint(0.5,0.5);
                self.Panel_20.addChild(card,100,168);

                var localArr = [];
                for(var i = 0;i < MJRoomModel.renshu;++i){
                    var tempArr = message.params.slice(1 + i * 2,(i + 1) * 2 + 1);
                    if(tempArr.length > 0){
                        localArr.push(tempArr);
                    }
                }
                ...
                if(localArr.length >= 1){
                    self.schedule(function(){
                        if(localArr[j][0]){
                            var seat = parseInt(localArr[j][0]);
                            var direct = MJRoomModel.getPlayerSeq("",MJRoomModel.mySeat,seat);
                            if(localArr[j][1] == 0){
                                self.showHaiDiGuoHu(direct);
                            }
                        }
                        ++j;
                    },0.3,localArr.length - 1,0.2);
                }
            },t);
        }
    },

    showHaiDiGuoHu:function(direct){
    ...
    onGDCSLastCircle:function(event){
        if(MJRoomModel.wanfa == GameTypeEunmMJ.YYMJ){
            ...
            //if(data.length % 3 != 2){
            //    MJRoomModel.yymjCloseCard = true;
            //}else{
            //    MJRoomModel.yymjInLastCircle = true;
            //}
        }
    },
    ...
    playSaiziAni:function(num1,num2){
        ...
        var saizi_1 = new cc.Sprite("res/res_mj/res_csmj/csmjRoom/saizi/Dice1_00" + num1 + ".png");
        var saizi_2 = new cc.Sprite("res/res_mj/res_csmj/csmjRoom/saizi/Dice2_00" + num2 + ".png");
    ...
    outCardTing:function(event){
        ...
        var info = data.info;
        var self = this;
        self.yymjTingData = null;
        setTimeout(function(){
            if (info){
                self.yymjTingData = info;//保存听牌数据
    ...
    onCheckResult:function(){
        if(this.resultData){
            var mc = new YYMJSmallResultPop(this.resultData);
            PopupManager.addPopup(mc);
        }
    },
    ...
    onJixuFromResult:function(){
        ...
        if(MJRoomModel.totalBurCount == MJRoomModel.nowBurCount
            || (MJRoomModel.wanfa == GameTypeEunmMJ.YYMJ && ClosingInfoModel.ext[24] == 1)){
    ...
    onOver:function(event){
        ...
        this.initTJMJBtn(false);//隐藏报听按钮
        this.removeHuPanel();//清除听牌层
        MJRoomModel.yymjCloseCard = false;
        MJRoomModel.yymjInLastCircle = false;
        MJRoomModel.nymjHasPlay = false;
        MJRoomModel.nymjTwoMJ = false;//宁远麻将只剩两个王牌的开关
        MJRoomModel.nymjGetData = false;
        MJRoomModel.yymjHasSet = false;
        //MJRoomModel.yymjIsThrowCard = false;//是否已经出牌
        this.yymjTingData = null;
        this.tingList.length=0;
        ...
        this.overTimeout = setTimeout(function(){//延迟弹出结算框
            var mc = new YYMJSmallResultPop(data);
            PopupManager.addPopup(mc);
    ...
    initData:function(){
        ...
        if(this.Panel_20.getChildByTag(168)){
            this.Panel_20.removeChildByTag(168);
        }
        ...
        for(var i=0;i<players.length;i++){
            ...
            if(p.ext.length>0){//长春麻将，听牌了
                ...
                if(p.seat == MJRoomModel.mySeat){//是自己
                    MJRoomModel.yymjTing = p.ext[7] == 1;//要不要锁
                }
            }

            var point = -1;
            if(MJRoomModel.wanfa == GameTypeEunmMJ.YYMJ && MJRoomModel.ext[15] == 1){
                point = p.ext[3];
            }
    ...
    showNiaoPanel:function(data){
        ...
        if(MJRoomModel.renshu === 2){
            birdList = data.birdAttr;
            if(birdList && birdList.length > 0){
                for(var i = 0;i<birdList.length;++i){
                    var dirBird = birdList[i];
                    var pos = cc.p(0,0);
                    pos.x = -320 + i*640;
                    pos.y = -30;
                    var vo = MJAI.getMJDef(dirBird.mjId);
                    var card = new CSMahjong(MJAI.getDisplayVo(1, 1), vo);
                    if(dirBird.awardSeat > 0){
    ...
    onLetOutCard:function(event){
        ...
        switch (action){
            ...
            case MJAction.CHU_PAI://出牌动作
                //if(MJRoomModel.yymjInLastCircle){
                //    MJRoomModel.yymjCloseCard = true;//开启锁牌
                //}
                //刷新一下玩家状态
                ...
                if(this.layouts[1] && seat==MJRoomModel.mySeat){// && MJRoomModel.yymjHasSet
                    this.layouts[1].setColorNormal();
                    //MJRoomModel.yymjIsThrowCard = true;
                }
                if(ids.length > 0){
    ...
    showJianTou:function(seat){
        ...
        if(seat != -1 && seat!=null){
            ...
            var jtDir = this.getWidget("jt"+jtSeq[direct-1]);
            if(!jtDir){
                return;
            }
            jtDir.visible = true;
