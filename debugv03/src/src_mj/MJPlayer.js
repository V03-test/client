...
var MJPlayer = BaseRoomPlayer.extend({
    ...
    ctor:function(vo,root,seq){
        ...
        this._jettons = [];
        this._isStart = false;
        this.Image_icon = ccui.helper.seekWidgetByName(root,"Image_icon"+seq);
        ...
        if(MJRoomModel.wanfa == GameTypeEunmMJ.CQXZMJ){
            this.tuoguanSp.y = 30;
            this.tuoguanSp.setLocalZOrder(999);
        }
        ...
        if(MJRoomModel.wanfa == GameTypeEunmMJ.JZMJ){
            this.showXszImg(vo.ext[8],seq);
        }

        if(!MJRoomModel.isClubGoldRoom() && (MJRoomModel.wanfa == GameTypeEunmMJ.DHMJ || MJRoomModel.wanfa == GameTypeEunmMJ.BSMJ)){//保山德宏不要擂台分
    ...
    showMoneyIcon:function(label){
        ...
        if(!icon_bjd){
            label.x += 15;
            var icon_bjd = new cc.Sprite("res/res_gold/goldPyqHall/img_13.png");
    ...
    showXszImg:function(isShow,seq){
        ...
        if(isShow){
            if(!img){
                ...
                if((seq == 4) || (MJRoomModel.renshu == 3 && seq == 3)){
    ...
    showPiaoFenImg:function(type){
        ...
        if(MJRoomModel.wanfa == GameTypeEunmMJ.ZOUMJ){
            pngUrl = "res/res_mj/res_dzmj/imgPao/img_pao_" + type + ".png";
        }
    ...
    showIcon:function(){
        ...
        this.userIconPop.showIcon(this.Image_icon,this._playerVo.icon,null,MJRoomModel.isMoneyRoom());
        return;
    ...
    chuPai:function(mjVo,delayTime){
        ...
        var mj = null;
        mj = new CSMahjong(MJAI.getDisplayVo(1, 1), mjVo);
        mj.setScale(1.15);
        mj.x = -3;
        mj.y = -5;
