...
var CDTLJSmallResultLayer = PKSmallResultPop.extend({

    ctor: function (data,isRePlay) {
        this.isRePlay = CDTLJRoomModel.replay;
        this.data = data;
    ...
    selfRender: function () {
        ...
        if(CDTLJRoomModel.renshu == 2){
            this.getWidget("player3").visible = false;
        }else if(CDTLJRoomModel.renshu == 3){
        ...
        this.dt = 0;
        this.start = 3;
        if(CDTLJRoomModel.isGameSite>0)
            this.scheduleUpdate();
        ...
        this.getWidget("Label_roomnum").setString("房间号:"+CDTLJRoomModel.tableId);
        var jushuStr = "第" + CDTLJRoomModel.nowBurCount + "局";
        this.getWidget("Label_jushu").setString(jushuStr);
        //玩法显示
        var wanfaStr = "";
        if(!this.isRePlay){
            wanfaStr = ClubRecallDetailModel.getCDTLJWanfa(CDTLJRoomModel.intParams,true);
        }
        ...
        // cc.log("CDTLJRoomModel.creditConfig =",CDTLJRoomModel.creditConfig);
        if ((CDTLJRoomModel.nowBurCount == CDTLJRoomModel.totalBurCount) || (this.data.ext[21] == 1)) {
            xipai_btn.visible = false;
        }else{
            xipai_btn.visible = CDTLJRoomModel.creditConfig[10] == 1;
        }
        var xpkf = CDTLJRoomModel.creditXpkf || 0;
    ...
    showPlayerMsg:function(widget,userData,index){
        ...
        if(user.boom == 1){
            ...
            var config = {"-3":"大倒","-2":"小倒","-1":"垮庄","1":"过庄","2":"小光","3":"大光"};
            var type = CDTLJRoomModel.getScoreType(this.data.ext[22],this.data.ext[23]);
            ...
            if((CDTLJRoomModel.intParams[13] == 1) || (CDTLJRoomModel.intParams[18] == 1 && this.data.ext[26] == 1)){//选了叫分进档
                if(jiaofen <= 30){
                    typeStr +="三档";
                }else if(jiaofen <= 50){
                    typeStr +="二档";
