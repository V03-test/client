...
var ApplyExitRoomPop = BasePopup.extend({
    ...
    selfRender: function () {
        ...
        for(var i=1;i <= 4;i++){
            ...
            this["ty"+i] = this.getWidget("Image_ty"+i);
            ...
            for (var j = 0; j < ApplyExitRoomModel.array.length; j++) {
                if (this.players[i - 1].userId ==  parseInt(ApplyExitRoomModel.array[j][0])){
                    ...
                    if(state == 2){
                        ...
                        this["ty"+i].loadTexture("res/res_gameCom/jiesan/tongyi.png");
                        this.tyNum += 1;
                        //this.updateBar();
                    }else if(state == 0){
                        this["ty"+i].visible = true;
                        ...
                        this["ty"+i].loadTexture("res/res_gameCom/jiesan/dengdai.png");
                    }
                    break;
                }
            }
        ...
        var elements = [];
        elements.push(RichLabelVo.createTextVo("玩家",cc.color("7E3102"),36));
        var nameStr = this.name;
        nameStr = UITools.truncateLabel(nameStr,5);
        elements.push(RichLabelVo.createTextVo(nameStr,cc.color("4974c9"),36));
        elements.push(RichLabelVo.createTextVo("申请解散房间，全部玩家同意后房间会被解散",cc.color("7E3102"),36));
