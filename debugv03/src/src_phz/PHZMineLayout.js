...
var PHZMineLayout = {
    ...
    getCardOffXForYZCHZ:function(){
        ...
        if (PHZSetModel.zpdx == 3 || PHZSetModel.zpdx == 4 ){
            w = w * (cc.winSize.width/SyConfig.DESIGN_WIDTH +0.06*(cc.winSize.width/SyConfig.DESIGN_WIDTH));
        }
        return w
    },
    ...
    reSetCardPos:function(){
        ...
        var initX = (winSize.width - w*result.length)/2 + 80;
            ...
            for(var j=0;j<array.length;j++){
                ...
                if(count<nowCards){
                    ...
                    if(GameTypeEunmZP.XPPHZ == PHZRoomModel.wanfa){
                        card = new XPPHZCard(PHZAI.getDisplayVo(1,1),array[j]);
                    }else{
