...
var NXGHZRoom = BaseLayer.extend({ //BaseLayer BaseRoom
    ...
    selfRender:function(){
        ...
        this.Panel_20.addChild(this.roomName_label, 0);
        ...
        this.addCustomEvent(SyEvent.YYWHZ_SHENPAI,this,this.onYYWHZShenPai);
        this.addCustomEvent(SyEvent.YYWHZ_YAOPAI,this,this.onYYWHZYaoPai);
        ...
        ClubRecallDetailModel.getSpecificWanfa(PHZRoomModel.intParams)
    ...
    //神牌
    onYYWHZShenPai:function(event){
        this.isDelay = true;
        var message = event.getUserData();
        var paiData = this.shenPaiData = parseInt(message.params[0]);
        cc.log("this.shenPaiData1",this.shenPaiData)
        var animName = ""
        if(paiData%2 == 0){
            animName = "shuangsheng"
        }else{
            animName = "dansheng"
        }
        this.addAnimation(animName,paiData)
    },
    //腰牌
    onYYWHZYaoPai:function(event){
        var message = event.getUserData();
        var paiData = message.params[0];
        var animName = ""
        if(parseInt(paiData)%2 == this.shenPaiData%2){
            animName = "yousheng"
        }else{
            animName = "meisheng"
        }
        //cc.log("onYYWHZYaoPai",paiData,parseInt(paiData)%2,this.shenPaiData,this.shenPaiData%2,animName)
        setTimeout(function(){
            this.cleanChuPai()
            this.addAnimation(animName,paiData)
        }.bind(this),700);
    ...
    addAnimation:function(animName,paiData){
        ...
        //创建相同卡牌来做动作
        var oPanel = this.getWidget("oPanel1")
        var cp1 = this.getWidget("cp1")
        cp1.visible = true;
        ...
        shenAnim.play();
        PHZRoomSound.actionSound(PlayerModel.userId,animName);
    },
    ...
    initwanfaImg:function(){
        ...
        var bgHeigh = 45 + wanfaArr.length * 38;
        if (this.Panel_20.getChildByName("wanfaImg")){
...
/*        var mc = new ChatPop("res/phzChat.json");
    ...
    calcWifi:function(){
        ...
        if (PHZRoomModel.is4Ren()) {
            ...
            this.battery.setTextureRect(cc.rect(0, 0, batteryNum, 30));
        } else {
            var batteryNum = Math.ceil(SdkUtil.getBatteryNum()/100*43);
            this.Label_batteryPer.setString(SdkUtil.getBatteryNum()+"%");
            this.battery.setTextureRect(cc.rect(0, 0, batteryNum, 30));
    ...
    initData:function(){
        this.roomName_label.setString(PHZRoomModel.roomName);
        if(GameTypeEunmZP.YYWHZ == PHZRoomModel.wanfa && PHZRoomModel.nowBurCount == 1) {
            this.Image_phz.removeAllChildren(true)
        }
        ...
        if(isContinue){
            if(PHZRoomModel.wanfa == GameTypeEunmZP.YYWHZ){
                this.shenPaiData = PHZRoomModel.ext[3]
                this.showShenPai(PHZRoomModel.ext[3])
            }
        ...
        var message = {closingPlayers:[
            {userId:"120004",name:"lx1234",point:16,totalPoint:16,winCount:null,lostCount:null,maxPoint:null,seat:1,sex:1,
                icon:"",bopiPoint:null,cards:[36,5,25,63,21,31,28,7,73,43],isShuXing:null,renewStatus:null,
                firstCards:[10,60,59,42,36,5,25,63,21,72,30,31,41,28,65,47,7,73,33,43],dahus:[],dahuCounts:[],neiYuanNum:null,
                waiYuanNum:null,mcards:[{action:3,cards:[52,42,72],huxi:1},{action:5,cards:[20,10,30],huxi:1},{action:6,
                cards:[48,59,60],huxi:0},{action:14,cards:[63,73,43],huxi:1},{action:15,cards:[36,7,28],huxi:0},{action:5,
                cards:[11,21,31],huxi:1},{action:14,cards:[5,25],huxi:1}],zhuaHao:null,qingHao:6,waiHao:10,credit:null,
                winLoseCredit:null,commissionCredit:null},{userId:"120073",name:"lx7788",point:-16,totalPoint:-16,winCount:null,
                ostCount:null,maxPoint:null,seat:2,sex:1,icon:"http://thirdwx.qlogo.cn/mmopen/vi_32/Dv2csZTBP5wJupj6zKibIuaaZkwphVDBX012k4btO0wticrvROE4UhYkxAsF0hETSXgibtLgTFxu2DSNRzkDTsEFw/132",
                bopiPoint:null,cards:[35,69,6,70,39,27,78,29,15,79],isShuXing:null,renewStatus:null,firstCards:[35,69,75,6,70,66,39,71,77,23,40,57,27,20,32,78,29,15,79],dahus:[],dahuCounts:[],
                neiYuanNum:null,waiYuanNum:null,mcards:[{action:6,cards:[17,32,40],huxi:1},{action:6,cards:[74,75,66],huxi:0},
                    {action:5,cards:[47,77,57],huxi:1},{action:-1,cards:[35,69,6,70,39,27,78,29,15,79],huxi:0}],zhuaHao:null,qingHao:0,
                waiHao:0,credit:null,winLoseCredit:null,commissionCredit:null}],isBreak:0,wanfa:801,ext:[855794,"120073,2020-12-10 11:11:15",801,0,1,0,2,0,0,0,1,0,0,0],matchExt:[],
            leftCards:[26,19,4,24,13,54],tun:null,fan:1,huxi:16,huSeat:1,huCard:11,totalTun:16,fanTypes:[],isRenew:null,renewCount:null,
            pointRecord:null,startLeftCards:[17,56,74,52,3,8,16,48,64,37,14,44,38,67,11,26,19,4,24,13,54,2,50,12,46,61,76,18,53,34,45,55,62,1,80,68,58,9,22,49,51],
            groupLogId:0}

        //ClosingInfoModel.init(message);
        //var mc = new NXGHZSmallResultPop(message);
        //PopupManager.addPopup(mc);
    },

    showShenPai:function(data){
        var imgStr = ""
        if(data%2 == 0){
            imgStr = "shuangshen"
        }else{
        ...
        var shenpai = this.Image_phz.getChildByName("shenpai")
        if(!shenpai){
            shenpai = new cc.Sprite("res/res_phz/yywhz/"+imgStr+".png");
            shenpai.setName("shenpai")
            shenpai.x = this.Image_phz.width/2
            shenpai.y = -210
            this.Image_phz.addChild(shenpai,-1);
        }else{
    ...
    onPengPai:function(obj,isAlert){
        ...
        switch (temp){
            ...
            case NXGHZAction.GUO:
                ...
                if(isHu){
                    PHZAlertPop.show("当前为可胡牌状态，确定要过吗？",function(){
            ...
            case NXGHZAction.CHI:
                ...
                if(isHu){
                    PHZAlertPop.show("当前为可胡牌状态，确定要吃吗？",function(){
                        self.chooseChi()
                    },function(){});
                }else{
                    self.chooseChi()
    ...
    displayChiSelect:function(cardsArray,result,needTimes,curTime){
        ...
        var itemW = 120;
        var width = 120+(result.length-1)*itemW;
...
//特殊处理大于11个吃选择的情况
    ...
    startGame:function(event){
        ...
        var self = this
        var delayTime = 0
        if(GameTypeEunmZP.YYWHZ == PHZRoomModel.wanfa && PHZRoomModel.nowBurCount == 1){//延迟时间，第一局需要播放动画
            delayTime = 2000;
        }
        setTimeout(function(){
            self.startSendCard(p)
        },delayTime);
    },

    startSendCard:function(p){
        if(GameTypeEunmZP.YYWHZ == PHZRoomModel.wanfa){
            this.showShenPai(this.shenPaiData)
        }
        this.isDelay = false;
        this.cleanChuPai();
        /** 庄家摸牌 */
    ...
    onLetOutPai:function(message){
        ...
        var self = this
        var delayTime = this.isDelay ? 2000 : 0
        setTimeout(function(){
            this.isDelay = false
    ...
    onInvite:function(){
        ...
        if(PHZRoomModel.wanfa == GameTypeEunmZP.NXGHZ){
            obj.description=clubStr + csvhelper.strFormat("南县鬼胡子，{0}局，{1}",PHZRoomModel.totalBurCount ,wanfaDesc);
        }else if(PHZRoomModel.wanfa == GameTypeEunmZP.YYWHZ){
            obj.description=clubStr + csvhelper.strFormat("益阳歪胡子，{0}局，{1}",PHZRoomModel.totalBurCount ,wanfaDesc);
        }
    ...
    showJianTou:function(seat,isTing){
        ...
        if(seat > 0){
            ...
            if(PHZRoomModel.renshu==4){
                ...
                coords = {1:{x:150,y:200},2:{x:300,y:840}};
            }
