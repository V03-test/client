...
var PHZRePlayModel = {
    ...
    init:function(data,isDaiKai){
        ...
        if(data.generalExt){
            ...
            PHZRoomModel.intParams = ext.intParams;
        }
        ...
        PHZRoomModel.renshu = data.resList.length;
        PHZRePlayModel.renshu = data.resList.length;
        ...
        for(;i<playLogs.length;i++){
            ...
            if(temp.length>=3){
                if(parseInt(temp[1])==12){
                    var key = this.steps.length - 1;
                    if(!this.optData[key])this.optData[key] = [];
                    var opt = {seat:temp[0],act:temp[2].split(",")};
                    this.optData[key].push(opt);
                    continue;
                }
    ...
    getCurSeatAct:function(seat,isBegin){
        var key = isBegin?-1:this.step;

        if(this.optData[key]){
            for(var i = 0;i<this.optData[key].length;++i){
                if(this.optData[key][i].seat == seat){
                    return this.optData[key][i].act;
                }
            }
        }
        return null;
