...
/*        var mc = new ChatPop("res/phzChat.json");
    ...
    displayChiSelect:function(cardsArray,result,needTimes,curTime){
        ...
        for(var i=0;i<result.length;i++){
            ...
            for(var j=0;j<array.length;j++){
                ...
                var scale = 0.8;
                // phz.scale=scale;
                phz.x = (innerbg.width-phz.width*scale)/2 - 7;
    ...
    onLetOutPai:function(message){
        ...
        if(PHZRoomModel.wanfa == GameTypeEunmZP.LDS || PHZRoomModel.wanfa == GameTypeEunmZP.YZCHZ){//落地扫王闯之类的胡牌操作特殊处理下
            if(action >= 15 && action <= 20)action = PHZAction.HU;
        }

        if(PHZRoomModel.wanfa == GameTypeEunmZP.AHPHZ){//安化跑胡子
    ...
    showJianTou:function(seat,isTing){
        ...
        if(seat > 0){
            ...
            if(PHZRoomModel.renshu==4){
                ...
                coords = {1:{x:200 + (SyConfig.DESIGN_WIDTH-cc.winSize.width)/2,y:190},2:{x:200 + (cc.winSize.width - SyConfig.DESIGN_WIDTH)/2,y:720}};
            }
