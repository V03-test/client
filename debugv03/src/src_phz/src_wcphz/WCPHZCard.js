 ...
 */
var WCPHZCard = ccui.Widget.extend({
    ...
    maxY:885,
    minY:-18,
    maxX:1782 + (cc.winSize.width-SyConfig.DESIGH_WIDTH)/2,
    minX:16,
    numberImg:null,
    isRecord:false,
    hasTouchBegin:false,
        ...
        if(type == ccui.Widget.TOUCH_BEGAN){
            ...
            if(isMyTurn && PHZRoomModel.selfAct.length==0){
                if (PHZRoomModel.getTouchCard() != this._cardVo.v){
                    ...
                    if(PHZRoomModel.wanfa == GameTypeEunmZP.WHZ || PHZRoomModel.wanfa == GameTypeEunmZP.WCPHZ
            ...
            if(this.y>=PHZSetModel.cardTouchend){//拖出去了松开，并且超过出牌线，直接出牌
                if(isMyTurn && PHZRoomModel.selfAct.length==0 && !this._cardVo.grayNoChu &&
                    !((PHZRoomModel.wanfa == GameTypeEunmZP.LDS || PHZRoomModel.wanfa == GameTypeEunmZP.YZCHZ) && this._cardVo.n == 11)){//落地扫王牌不能出
                    // this.setNormalColor();
                    PHZRoomModel.outCardsX = this.x;
                    PHZRoomModel.outCardsY = this.y;
                    PHZRoomModel.isSelfOutCard = true;
                    PHZRoomModel.simulateLetOutCard(this._cardVo.c,JSON.stringify(WCPHZMineLayout.curSortedData));
                }else{
                ...
                var isArrived = false;
                var isInsert = false;
                for(var i=0;i<WCPHZMineLayout.coords.length;i++){
                    var coord = WCPHZMineLayout.coords[i];
                    var tCardListLength = WCPHZMineLayout.curSortedData[i].length;
                    ...
                    if(Math.abs(this.x-coord.x)<67 && Math.abs(this.y-coord.y)<241){
                        ...
                        if(Math.abs(this.x-coord.x)<67 && Math.abs(this.y-coord.y)<241){
                            if(WCPHZMineLayout.curSortedData[i].length<=4){
                                this.setLocalZOrder(2 - insertIndexY );
                                WCPHZMineLayout.reSortAndDisplay(this._cardVo,i,isInsert,insertIndexY);
                                isArrived = true;
                            }
                ...
                if(!isArrived){
                    if(this.x>WCPHZMineLayout.coords[WCPHZMineLayout.coords.length-1].x+66 && WCPHZMineLayout.curSortedData.length<10){
                        WCPHZMineLayout.reSortAndDisplay(this._cardVo,-1,false);
                    }else if(this.x<WCPHZMineLayout.coords[0].x-66 && WCPHZMineLayout.curSortedData.length<10){
                        WCPHZMineLayout.reSortAndDisplay(this._cardVo,-2,false);
                        //}else{
                        //    this.setLocalZOrder(this.initZorder);
                        //    this.x = this.initX;
                        //    this.y = this.initY;
    ...
    diplay:function(action){
        ...
        if(same==1&&direct==1&&place==1){//大于等于3个一样的 加一层遮罩
            if(PHZRoomModel.wanfa != GameTypeEunmZP.LDS && PHZRoomModel.wanfa != GameTypeEunmZP.YZCHZ){//落地扫不让加置灰，说是看着不爽，我了个去
                bgImg.setColor(cc.color(175,175,175));
            }
        }
