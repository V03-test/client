 ...
 */
var WCPHZRoom = BaseLayer.extend({ //BaseLayer BaseRoom
    layouts:{},
    tag_btn_chi:601,
    tag_btn_liu:602,
    tag_btn_other:501,
    ...
    selfRender:function(){
        ...
        if(PHZRoomModel.wanfa==GameTypeEunmZP.XTPHZ || PHZRoomModel.wanfa==GameTypeEunmZP.XXPHZ || PHZRoomModel.wanfa==GameTypeEunmZP.XXGHZ
            ...
            ||  PHZRoomModel.wanfa==GameTypeEunmZP.WCPHZ
    ...
    tcmjStartPiaoFen:function(event){
        ...
        this.showTCMJPiaoBtn(true);
    },

    showTCMJPiaoBtn:function(isBool){
        if(isBool){
            if(!this.tcmjPiaoBtnNode){
                ...
                var imgArr = ["bupiao.png","1fen.png","2fen.png","3fen.png","4fen.png","5fen.png"];
                var tagArr = [0,1,2,3,4,5];
                var offsetX = 300;
                var startX = -(imgArr.length - 1)/2*offsetX;
                for(var i = 0;i<imgArr.length;++i){
                    var img = "res/res_mj/res_csmj/csmjRoom/" + imgArr[i];
    ...
    tcmjFinishPiaoFen:function(event){
        ...
        if(PHZRoomModel.intParams[20] == 0) return
    ...
    showWaitSelectPiao:function(isShow){
        cc.log("showWaitSelectPiao",isShow)
        if(isShow){
            if(!this.waitPiaoImg){
                this.waitPiaoImg = new cc.Sprite("res/res_mj/res_csmj/csmjRoom/word_piaofen.png");
                this.waitPiaoImg.setPosition(cc.winSize.width/2 + 50,cc.winSize.height/2);
                this.addChild(this.waitPiaoImg);
            }
    ...
    checkTingList:function(isTrue) {
        ...
        if (PHZRoomModel.nextSeat == PHZRoomModel.mySeat || isTrue){
            ...
            var sourceArray = ArrayUtil.clone(WCPHZMineLayout.getCurVoArray());
            var arr = [];
            //筛选出重复的牌
            var needCheckArray = [];
    ...
    getcheckHuIdArr:function() {
        ...
        //获取自己的手牌
        if (WCPHZMineLayout){
            var arr = WCPHZMineLayout.getHandCardData();
            if (arr && arr.length > 0){
                for(var i=0;i<arr.length;i++){
    ...
    getRedCardsNum:function(){  
        ...
        var handCards = WCPHZMineLayout.getCurVoArray();
    ...
    outCardTing:function(event){
        ...
        var data = event.getUserData();
        var info = data.info;
        WCPHZMineLayout.outCardTingPai(info);
    ...
    showTingPai:function(huList){
        ...
        if (huList && huList.length > 0 ){
            ...
            for(var j=0;j<huList.length;j++){
                var cardVo = PHZAI.getPHZDef(huList[j]);
                var id = cardVo.n;
...
/*        var mc = new ChatPop("res/phzChat.json");
    ...
    initData:function(){
        ...
        WCPHZMineLayout.setRoot(this.getWidget("minePanel"));
        if(!isContinue)
            WCPHZMineLayout.clean();
        this.btnPanel.visible = false;
        ...
        for(var i=0;i<players.length;i++){
            ...
            if(PHZRoomModel.intParams[20] != 0) {
                this._players[p.seat].showPiaoFenImg(p.ext[8]);
            }
            if(!isContinue){
                if(PHZRoomModel.intParams[20] != 0&&p.ext[8] >= 0 && p.seat == PHZRoomModel.mySeat){
                    this.showWaitSelectPiao(true);
                }
                if(p.status && !p.ext[9])
                    cardPlayer.onReady();
    ...
    onPengPai:function(obj,isAlert){
        ...
        switch (temp){
            ...
            case PHZAction.PENG:
                ...
                if(isHu){
                    PHZAlertPop.show("当前为可胡牌状态，确定要碰吗？",function(){
                        PHZRoomModel.sendPlayCardMsg(temp,[]);
                    },function(){});
                }else{
                    PHZRoomModel.sendPlayCardMsg(temp,[]);
    ...
    chooseChi:function(){
        if(this.btnPanel.getChildByTag(this.tag_chi_select))
            return;
        var sourceArray = WCPHZMineLayout.getCurVoArray();
    ...
    getLiuCards:function(){
        ...
        var handCards = WCPHZMineLayout.cards;
        for (var i = 0; i < handCards.length; ++i) {
    ...
    startGame:function(event){
        ...
        || GameTypeEunmZP.XTPHZ == PHZRoomModel.wanfa || GameTypeEunmZP.WCPHZ == PHZRoomModel.wanfa
    ...
    refreshButton:function(selfAct){
        ...
        if(selfAct.length>0){
            ...
            if(btnDatas.length>0 && isShowBtn){
                ...
                var isShowGuo = true;
                if(PHZRoomModel.wanfa == GameTypeEunmZP.LDS && isHu){//落地扫无王必胡和放炮必胡
    ...
    onLetOutCard:function(event){
        ...
        if(seat==PHZRoomModel.mySeat&&actType==2&&action==0&&ids.length>0&&!simulate){
            if (!WCPHZMineLayout.isHasHardCard(ids[0])){
    ...
    onLetOutPai:function(message){
        ...
        if(actType!=0){//出牌动作 actType1为摸牌 2为出牌
            if(ids.length>0){
                ...
                || isTuoguan){
                    WCPHZMineLayout.delOne(cardTransData[0],true);
                }
            ...
            if(action==PHZAction.LONG_BU_ZI) {//龙补字
                ...
                if(ids.length>0 && (action==4||!this.kehushizhong)){
                     this.kehushizhong  = false;
                        ...
                        for(var i=0;i<ids.length;i++) {
                            WCPHZMineLayout.delOne(PHZAI.getPHZDef(ids[i]),(i==ids.length-1),isZaiPao);
                        }
                }
            }
    ...
    onGetWangPai:function(message,voArray){
        ...
        if(seat==PHZRoomModel.mySeat){
            WCPHZMineLayout.handleLongBuZi(vo);
        }
        PHZRoomSound.letOutSound(userId,vo);
        WCPHZMineLayout.outCardTingPai(this.outTingInfo);
        var self = this;
        if(voArray.length>0){
            setTimeout(function(){
                self.cleanChuPai();
    ...
    showJianTou:function(seat,isTing){
        ...
        if(seat > 0){
            ...
            if(PHZRoomModel.renshu==4){
                ...
                coords = {1:{x:200 + (SyConfig.DESIGN_WIDTH-cc.winSize.width)/2,y:190},2:{x:200 + (cc.winSize.width - SyConfig.DESIGN_WIDTH)/2,y:800}};
            }
    ...
    initCards:function(direct,p1Mahjongs,p2Mahjongs,p3Mahjongs,extMahjongs,seat,isMoPai){
        ...
        if(direct==1){
            WCPHZMineLayout.initData(p1Mahjongs,this.getWidget("minePanel"));
            this.checkGrayCard();
        }
    },
    ...
    checkGrayCard:function(){
        if(PHZRoomModel.wanfa == GameTypeEunmZP.WHZ){
            ...
            if(moldCards){
                ...
                var cards = WCPHZMineLayout.cards;
                for(var i = 0;i<cards.length;++i){
    ...
    updateSetZpdx:function(){
        if (WCPHZMineLayout){
            WCPHZMineLayout.changeHandCardSize();
        }
        if (this.lastCard) {
            this.showTipCard()
        }
    ...
    updateSetZpxz:function(){
        ...
        if (WCPHZMineLayout){
            WCPHZMineLayout.changeHandCardTextrue();
        }
        if (PHZRoomEffects){
    ...
    updateSetPmxz:function(){
        ...
        if (WCPHZMineLayout){
            WCPHZMineLayout.changeHandCardBg();
        }
        if (PHZRoomEffects){
    ...
    updateBgColor:function(){
        ...
        var wanfaUrl = "";
        if (PHZRoomModel.wanfa == GameTypeEunmZP.WCPHZ){
            gameTypeUrl = "res/res_phz/wanfaImg/gametype1_wcphz.png";
        }else{
        ...
        if (PHZSetModel.zmbj == 1){
            ...
            if (PHZRoomModel.wanfa == GameTypeEunmZP.WCPHZ){
                gameTypeUrl = "res/res_phz/wanfaImg/gametype2_wcphz.png";
            }
            ...
            if (PHZRoomModel.wanfa == GameTypeEunmZP.WCPHZ){
                gameTypeUrl = "res/res_phz/wanfaImg/gametype3_wcphz.png";
            }
