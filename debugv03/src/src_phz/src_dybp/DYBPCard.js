 ...
 */
var DYBPCard = ccui.Widget.extend({
        ...
        if(type == ccui.Widget.TOUCH_BEGAN){
            ...
            if(isMyTurn && PHZRoomModel.selfAct.length==0){
                ...
                if (PHZRoomModel.getTouchCard() != this._cardVo.v){
                    ...
                    if(PHZRoomModel.wanfa == GameTypeEunmZP.WHZ || PHZRoomModel.wanfa == GameTypeEunmZP.LDS
                        ...
                        || PHZRoomModel.wanfa == GameTypeEunmZP.YJGHZ || PHZRoomModel.wanfa == GameTypeEunmZP.NXGHZ || PHZRoomModel.wanfa == GameTypeEunmZP.YYWHZ){
                        SyEventManager.dispatchEvent(SyEvent.SHOW_TING,this._tingList);
                    }else{
                        SyEventManager.dispatchEvent(SyEvent.PHZ_DEAL_CARD,this._cardVo);
                    }
            ...
            if(this.y>=PHZSetModel.cardTouchend){//拖出去了松开，并且超过出牌线，直接出牌
                if(isMyTurn && PHZRoomModel.selfAct.length==0 && this._cardVo.v!=0 && !this._cardVo.isGray){
                    ...
                    PHZRoomModel.simulateLetOutCard(this._cardVo.c,JSON.stringify(DYBPMineLayout.curSortedData));
                }else{
                ...
                var isArrived = false;
                var isInsert = false;
                for(var i=0;i<DYBPMineLayout.coords.length;i++){
                    var coord = DYBPMineLayout.coords[i];
                    var tCardListLength = DYBPMineLayout.curSortedData[i].length;
                    ...
                    if(Math.abs(this.x-coord.x)<67 && Math.abs(this.y-coord.y)<241){
                        ...
                        if(Math.abs(this.x-coord.x)<67 && Math.abs(this.y-coord.y)<241){
                            if(DYBPMineLayout.curSortedData[i].length<=4){
                                this.setLocalZOrder(2 - insertIndexY );
                                DYBPMineLayout.reSortAndDisplay(this._cardVo,i,isInsert,insertIndexY);
                                isArrived = true;
                            }
                ...
                if(!isArrived){
                    if(this.x>DYBPMineLayout.coords[DYBPMineLayout.coords.length-1].x+66 && DYBPMineLayout.curSortedData.length<10){
                        DYBPMineLayout.reSortAndDisplay(this._cardVo,-1,false);
                    }else if(this.x<DYBPMineLayout.coords[0].x-66 && DYBPMineLayout.curSortedData.length<10){
                        DYBPMineLayout.reSortAndDisplay(this._cardVo,-2,false);
                        //}else{
                        //    this.setLocalZOrder(this.initZorder);
                        //    this.x = this.initX;
                        //    this.y = this.initY;
    ...
    diplay:function(action){
        ...
        if (this._cardVo.ishu){ //当前胡的这张牌
            scale = 0.89;
            bgPng = "small_half_back"+pmType+".png";
        }
