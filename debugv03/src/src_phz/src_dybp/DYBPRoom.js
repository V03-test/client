var DYBPRoom = BaseLayer.extend({ //BaseLayer BaseRoom
    layouts:{},
    tag_btn_chi:601,
    tag_btn_liu:602,
    tag_btn_other:501,
    ...
    checkTingList:function(isTrue) {
        ...
        if (PHZRoomModel.nextSeat == PHZRoomModel.mySeat || isTrue){
            ...
            var sourceArray = ArrayUtil.clone(DYBPMineLayout.getCurVoArray());
            var arr = [];
            //筛选出重复的牌
            var needCheckArray = [];
    ...
    getcheckHuIdArr:function() {
        ...
        //获取自己的手牌
        if (DYBPMineLayout){
            var arr = DYBPMineLayout.getHandCardData();
            if (arr && arr.length > 0){
                for(var i=0;i<arr.length;i++){
    ...
    getHuList:function(_voArray,_handCards,_isPaoHu,_tingCards) {
        ...
        for(var i=0;i<_voArray.length;i++){
            ...
            var qihuHuxi = PHZRoomModel.intParams[32];
    ...
    getRedCardsNum:function(){
        ...
        var handCards = DYBPMineLayout.getCurVoArray();
    ...
    outCardTing:function(event){
        ...
        DYBPMineLayout.outCardTingPai(info);
    ...
    updateRoomInfo:function(){
        ...
        if(this.lable_GameName){
            this.lable_GameName.setString("大字剥皮");
        }
    ...
    chooseChi:function(){
        if(this.btnPanel.getChildByTag(this.tag_chi_select))
            return;
        var sourceArray = DYBPMineLayout.getCurVoArray();
    ...
    displayChiSelect:function(cardsArray,result,needTimes,curTime){
        ...
        for(var i=0;i<result.length;i++){
            ...
            for(var j=0;j<array.length;j++){
                var phz = new DYBPCard(PHZAI.getDisplayVo(1,2),array[j]);
                var scale = 0.8;
                // phz.scale=scale;
    ...
    getLiuCards:function(){
        ...
        var handCards = DYBPMineLayout.cards;
        for (var i = 0; i < handCards.length; ++i) {
    ...
    showTipCard:function(){
        ...
        if (this.lastLetOutMJ && this.lastLetOutMJ > 0){
            ...
            var phz = this.lastCard = new DYBPCard(PHZAI.getDisplayVo(1,1),lastCard,null,true);
            var scaleY = 0.75;
            var scaleX = 0.9;
            phz.scaleY = scaleY;
            phz.scaleX = scaleX;
    ...
    onLetOutCard:function(event){
        ...
        if(seat==PHZRoomModel.mySeat&&actType==2&&action==0&&ids.length>0&&!simulate){
            if (!DYBPMineLayout.isHasHardCard(ids[0])){
        ...
        if(actType!=0){//出牌动作 actType1为摸牌 2为出牌
            if(ids.length>0){
                ...
                if(actType==2 && (seat==PHZRoomModel.mySeat || (seat == PHZRoomModel.bankerSeat && PHZRoomModel.mySeat == PHZRoomModel.sxSeat))
                    || isTuoguan){
                    DYBPMineLayout.delOne(cardTransData[0],true);
                }
            ...
            if(action==PHZAction.LONG_BU_ZI) {//龙补字
                ...
                if(ids.length>0 && (action==4||!this.kehushizhong)){
                    ...
                    for(var i=0;i<ids.length;i++) {
                        DYBPMineLayout.delOne(PHZAI.getPHZDef(ids[i]),(i==ids.length-1),isZaiPao);
                    }
                }
            }
    ...
    onGetWangPai:function(message,voArray){
        ...
        if(seat==PHZRoomModel.mySeat){
            DYBPMineLayout.handleLongBuZi(vo);
        }
        PHZRoomSound.letOutSound(userId,vo);
        DYBPMineLayout.outCardTingPai(this.outTingInfo);
        var self = this;
        if(voArray.length>0){
            setTimeout(function(){
                self.cleanChuPai();
    ...
    onInvite:function(){
        ...
        if(PHZRoomModel.wanfa==GameTypeEunmZP.DYBP){
            obj.description=clubStr + csvhelper.strFormat("大字剥皮，{0}人，{1}",PHZRoomModel.renshu  , wanfaDesc);
        }
        obj.shareType=1;
        //SdkUtil.sdkFeed(obj);
        ShareDTPop.show(obj);
    ...
    initCards:function(direct,p1Mahjongs,p2Mahjongs,p3Mahjongs,extMahjongs,seat,isMoPai){
        ...
        if(direct==1){
            DYBPMineLayout.initData(p1Mahjongs,this.getWidget("minePanel"));
            this.checkGrayCard();
        }
    },
    ...
    checkGrayCard:function(){
        ...
        //            }
        //        }
        //        var cards = DYBPMineLayout.cards;
    ...
    updateSetZpdx:function(){
        if (DYBPMineLayout){
            DYBPMineLayout.changeHandCardSize();
        }
        if (this.lastCard) {
            this.showTipCard()
        }
    ...
    updateSetZpxz:function(){
        ...
        if (DYBPMineLayout){
            DYBPMineLayout.changeHandCardTextrue();
        }
        if (PHZRoomEffects){
    ...
    updateSetPmxz:function(){
        ...
        if (DYBPMineLayout){
            DYBPMineLayout.changeHandCardBg();
        }
        if (PHZRoomEffects){
    ...
    updateSetIscp:function(){
        if (DYBPMineLayout){
            DYBPMineLayout.setCardOffY();
        }
    },
    updateBgColor:function(){
        var bgTexture = "res/res_phz/roombg/room_bg1.jpg";
        var gameTypeUrl = "res/res_phz/wanfaFront/dzbp.png";
        ...
        for(var n=0;n<PHZRoomModel.renshu;n++) {
            ...
            for (var i = 0; i < result.length; i++) {
                ...
                for (var j = 0; j < cardArray.length; j++) {
                    if(seq!=1) {
                        var scale = 0.9;
                        var card = new DYBPCard(PHZAI.getDisplayVo(seq, 2), cardArray[j]);
