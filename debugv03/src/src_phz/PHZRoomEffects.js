...
var PHZRoomEffects = {
    ...
    chuPai:function(root,phzVo,actType,renshu,seq,oPanel,callBack){
        ...
        if (oPanel){
            var kuang1 = this.qiCard = this.getSprite(kuangText);
    ...
    chiAnimate:function(ids,root,direct,callback,action,burCount){
        ...
        while(voArray.length>0){
            ...
            for(var i=0;i<array.length;i++){
                var vo = array[i];
                var phz = new AHPHZCard(PHZAI.getDisplayVo(1,2),vo);
                phz.scale = cardScale;
        ...
        if(PHZRoomModel.renshu==4){
            coords1 = {1:{x:736,y:532},2:{x:1470,y:739},3:{x:826,y:855},4:{x:176,y:700}};
        }else if(PHZRoomModel.renshu==3){
            coords1 = {1:{x:674,y:702},2:{x:1470,y:739},3:{x:20,y:646}};
        }else{
            coords1 = {1:{x:698,y:640},2:{x:1470,y:739}};
        }
