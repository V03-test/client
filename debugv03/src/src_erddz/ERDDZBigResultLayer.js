...
var ERDDZBigResultLayer = PKBigResultPop.extend({
    ...
    refreshSingle:function(widget,user){
        ...
        ccui.helper.seekWidgetByName(widget,"Label_title1").setString("地主次数");
        ccui.helper.seekWidgetByName(widget,"Label_txt1").setString(""+(user.actionCounts[0] || 0));

        ccui.helper.seekWidgetByName(widget,"Label_title2").setString("赢家次数");
        ccui.helper.seekWidgetByName(widget,"Label_txt2").setString(""+(user.actionCounts[1] || 0));

        ccui.helper.seekWidgetByName(widget,"Label_title3").setString("炸弹次数");
        ccui.helper.seekWidgetByName(widget,"Label_txt3").setString(""+(user.actionCounts[2] || 0));
    ...
    selfRender: function () {
        ...
        //显示部分房间信息
        var wanfaDesc = ClubRecallDetailModel.getSpecificWanfa(ERDDZRoomModel.intParams);;

        this.getWidget("Label_wanfa").setString("二人斗地主  "+wanfaDesc);


        var jushuStr = "第" + ERDDZRoomModel.nowBurCount + "/" + ERDDZRoomModel.totalBurCount + "局";
        this.getWidget("Label_jushu").setString(jushuStr);

        this.getWidget("Label_roomnum").setString("房号:" + ERDDZRoomModel.tableId);

        this.getWidget("Label_score").setString("");
        if (ERDDZRoomModel.isCreditRoom()){
            this.getWidget("Label_score").setString(ERDDZRoomModel.getCreditConfigStr());
        }

        if(ERDDZRoomModel.isClubGoldRoom()){
            this.getWidget("Label_score").setString(ERDDZRoomModel.getClubGlodCfg());
        }
    },

    onShareCard:function() {
        this.shareCard(ERDDZRoomModel, this.data, ClosingInfoModel.groupLogId);
    },
